...
// Windows 服务工具类
pub struct WindowsServiceUtil;

impl WindowsServiceUtil {
    // 查询服务状态，返回枚举类型
    ...
    pub fn start_service(service_name: &str) -> bool {
        match Self::with_service_handle(service_name, ServiceAccess::START) {
            Ok(service) => {
                // 先检查当前状态
                if let Ok(ServiceQueryResult::Running) = Self::query_service_status(service_name) {
    ...
    pub fn stop_service(service_name: &str) -> bool {
        match Self::with_service_handle(service_name, ServiceAccess::STOP) {
            Ok(service) => {
                // 先检查当前状态
                if let Ok(ServiceQueryResult::Stopped) = Self::query_service_status(service_name) {
    ...
    fn wait_for_service_state(
        ...
        while start_time.elapsed() < timeout {
            match WindowsServiceUtil::query_service_status(service_name) {
